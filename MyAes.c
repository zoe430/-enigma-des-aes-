...
void aes_polynomial_mul(unsigned char x[4], unsigned char y[4], unsigned char z[4])//aes_8bit_mul_mod_0x11B(unsigned int x, unsigned int y)
{
	...
	for (cou = 0; cou < 4; cou++)
	{
		T[cou] = (aes_8bit_mul_mod_0x11B(x[(3 - cou) % 4], y[0]) ^ aes_8bit_mul_mod_0x11B(y[1], x[(4 - cou) % 4]) ^ aes_8bit_mul_mod_0x11B(y[2], x[(5 - cou) % 4]) ^ aes_8bit_mul_mod_0x11B(y[3], x[(6 - cou) % 4]) );
	}
	memcpy(z, T, 4 );
}
